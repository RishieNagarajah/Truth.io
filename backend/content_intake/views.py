...
# OpenSource LLM-based approach
class ClaimSupport:
    def __init__(self, index, similarity_score, sentence):
        self.index = index
...
def process_content_2(request):
    ...
    # Step 2, Step 3 -> Sentence segmentation and sentence embeddings at the sentence level
    document = nlp_model_spaCy(request)
    cleaned_sentences = [sentence.text.strip() for sentence in document.sents if sentence.text.strip()]
    sentence_data = []
    all_sentence_embeddings = sentence_model.encode(cleaned_sentences, convert_to_numpy=True, show_progress_bar=True)
    for index, sentence in enumerate(cleaned_sentences):
        ...
        sentence_data.append(
            {
                'sentence': cleaned_sentence,
                'embedding': all_sentence_embeddings[index],
                'index': index
            }
        )
    ...
    for candidate in ranked_candidates:
        if (candidate['index'] not in assigned_indices and candidate['textrank_score'] >= 0.85):
            main_claims_info.append(
                { 
                    'sentence': candidate['sentence'],
                    'embedding': candidate['embedding'],
    ...
    for index, claim in enumerate(main_claims_info):
        ...
        for neighbor_data in sentence_data:
            ...
            is_within_proximity = (abs(neighbor_index - claim_index) <= 5)
            similarity_to_claim = cosine_similarity(neighbor_embedding, claim_embedding).flatten()[0]
            if (is_within_proximity and similarity_to_claim >= 0.25): #  and similarity_to_claim >= 0.65 and len(main_claims_info[index]['support']) <= 4
                new_item = ClaimSupport(neighbor_index, similarity_to_claim, neighbor_sentence)
                heapq.heappush(
                    main_claims_info[index]['support'], 
                    new_item
                )
                if (len(main_claims_info[index]['support']) > 4):
                    heapq.heappop(main_claims_info[index]['support']) 
                #assigned_indices.add(neighbor_index)
            main_claims_info[index]['support'].sort(key=lambda item: item.index)
    ...
    for index, claim in enumerate(main_claims_info):
        ...
        print("SUPPORTING DETAILS:")
        for support in claim['support']:
            print(f'Index: { support.index }')
            print(f'Sentence: { support.sentence }')
            print(f'Score: { support.similarity_score }')
